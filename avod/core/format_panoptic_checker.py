...
- ObjectLabel format is well-documented in the object_panoptic_utils in wavedata
- It is used more for encoding a generic label class.
- It is a format used in evaluation for Panoptic dataset.
- This format is useful for interfacing with obj_panoptic_utils operations. Note: most
  of the obj_panoptic_utils operation is not optimized for batch operations and it might
  be slow.
...
def check_object_label_format(input_data):
    ...
    print('check_object_label_format(input_data):')
    print('input_data = ', input_data)
    if isinstance(input_data, list):
        if not all(isinstance(x, obj_panoptic_utils.ObjectLabel) for x in input_data):
    ...
    elif isinstance(input_data, obj_panoptic_utils.ObjectLabel):
