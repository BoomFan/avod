...
from avod.core.mini_batch_panoptic_preprocessor import MiniBatchPreprocessor
...
class MiniBatchUtils:
    def __init__(self, dataset):
        ...
        ##############################
        # Parse PanopticUtils config
        ##############################
        self.panoptic_utils_config = dataset.config.panoptic_utils_config
        self._area_extents = self.panoptic_utils_config.area_extents
        self._anchor_strides = np.reshape(
            self.panoptic_utils_config.anchor_strides, (-1, 2))

        ##############################
        # Parse MiniBatchUtils config
        ##############################
        self.config = self.panoptic_utils_config.mini_batch_config
    ...
    def preprocess_rpn_mini_batches(self, indices):
        """Generates rpn mini batch info for the panoptic dataset
