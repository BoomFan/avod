...
from avod.datasets.panoptic.panoptic_dataset import PanopticDataset


class PanopticDatasetTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.fake_panoptic_dir = tests.test_path() + "/datasets/Panoptic/object"
    ...
    def test_data_loading(self):
        dataset = self.get_fake_dataset('train', self.fake_panoptic_dir)
    ...
    def test_data_splits(self):
        bad_config = DatasetBuilder.copy_config(DatasetBuilder.PANOPTIC_UNITTEST)

        # Test invalid splits
        bad_config.data_split = "bad"
        self.assertRaises(ValueError, PanopticDataset, bad_config)
        ...
        # Train split
        train_dataset = self.get_fake_dataset('train', self.fake_panoptic_dir)
        self.assertEqual(train_dataset.num_samples, 7)

        # Validation split
        validation_dataset = self.get_fake_dataset('val', self.fake_panoptic_dir)
        self.assertEqual(validation_dataset.num_samples, 6)

        # Train + validation split
        trainval_dataset = self.get_fake_dataset('trainval',
                                                 self.fake_panoptic_dir)
        self.assertEqual(trainval_dataset.num_samples, 13)

        # Test split
        test_dataset = self.get_fake_dataset('test', self.fake_panoptic_dir)
    ...
    def test_batch_loading(self):
        ...
        # Validation split
        dataset = self.get_fake_dataset('val', self.fake_panoptic_dir)

        batch = dataset.next_batch(3)
        self.assertEqual(len(batch), 3)

        # Testing split
        dataset = self.get_fake_dataset('test', self.fake_panoptic_dir)
